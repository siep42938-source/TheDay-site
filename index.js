...
const linkTg     = (token, tgId, u)    => api('/bot/link-tg',         { token, telegramId: String(tgId), telegramUsername: u });
...
bot.action(/^buy_(.+)$/, async ctx => {
  ...
  if (hasCoins) rows.splice(1, 0, [Markup.button.callback(`💎 Купить за монеты (${bal.balance} монет)`, `bal_${p.id}`)]);
  rows.push([Markup.button.callback('◀️ Назад', 'back_shop')]);
  await ctx.reply(
    `${p.emoji} *TheDay — ${p.name}*\n💰 Цена: *${p.price}₽*\n\n` +
    `💳 *FanPay* — оплати, пришли скриншот, получишь ключ\n` +
    (hasCoins ? `💎 *Монеты* — мгновенно (баланс: ${bal.balance})\n` : '') +
    `\nВыберите способ:`,
